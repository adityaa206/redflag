...
class _PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 8, f"M&A Cybersecurity Due Diligence  ·  {self._target}  ·  {self._scan_date}", ln=0, align="R")
    ...
    def footer(self):
        ...
        self.cell(0, 8, f"CONFIDENTIAL — Generated by RedFlag  ·  Page {self.page_no()}", align="C")
    ...
    def _finding_row(self, f, is_shade: bool):
        ...
        # Title (truncated)
        title = f.title if len(f.title) <= 52 else f.title[:49] + "…"
        ...
        # Host:Port
        host_str = f"{f.host or '—'}:{f.port}" if f.port else (f.host or "—")
        self.set_font("Helvetica", "", 7)
        self.set_text_color(*_MUTED)
...
def generate_pdf_report(
    ...
    if deal_killers:
        ...
        for f in deal_killers:
            ...
            pdf.cell(0, 5, f.title[:90], ln=True)
            pdf.set_x(10)
            pdf.set_font("Helvetica", "", 8)
            pdf.set_text_color(*_MUTED)
            host_str = f"{f.host}:{f.port}" if f.port else (f.host or "—")
            cve_str  = f.cve_id or "—"
            pdf.cell(0, 5, f"Host: {host_str}   CVE: {cve_str}   CVSS: {f.cvss_score:.1f}   Score: {f.risk_score:.1f}", ln=True)
            if f.override_reason:
                pdf.set_x(10)
                pdf.set_font("Helvetica", "I", 7.5)
                pdf.set_text_color(251, 191, 36)
                pdf.multi_cell(190, 4.5, f"Reason: {f.override_reason}")
            pdf.set_x(10)
            pdf.set_font("Helvetica", "", 7.5)
            pdf.set_text_color(*_TEXT)
            desc = f.description[:280] + ("…" if len(f.description) > 280 else "")
            pdf.multi_cell(190, 4.5, desc)
            pdf.set_x(10)
